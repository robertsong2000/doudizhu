...
| v1.1 | 2026-04-30 | Codex | 补充视觉升级、真实扑克牌牌面、牌桌沉浸感与可玩性验收要求 |
| v1.2 | 2026-04-30 | Codex | 补充角色头像、音乐音效、手牌一屏展示和点击响应优化 |
...
| F056 | 真实牌面视觉 | 手牌、桌面牌、地主牌使用接近真实扑克牌的视觉表达 | 每张牌必须包含左上角点数、花色符号、中心花色/王牌标识、圆角白色纸面和清晰红黑配色；背面牌使用独立纹理，不能只是纯色块或文字 |
| F057 | 牌桌沉浸感 | 游戏桌整体呈现真实牌桌氛围，突出中间出牌区和底部手牌区 | 桌面区域有绿色毡布质感、边框、阴影和层次；玩家、状态、操作区不互相遮挡 |
| F058 | 手牌可读性 | 真人手牌在不同数量下仍可读、可点选 | 17-20 张手牌在桌面宽度下尽量完整展示；窄屏允许横向滚动；选中态明显上移并高亮 |
| F059 | 身份头像 | 玩家身份确定后展示地主/农民头像 | 地主显示地主头像，两名农民显示农民头像；真人玩家和电脑玩家均同步更新 |
| F060 | 音乐音效 | 提供背景音乐与操作音效开关 | 支持开启/关闭背景音乐；叫地主、出牌、不出、炸弹、胜负等关键动作有音效反馈 |
...
本迭代采用“沉浸式木质牌桌 + 绿色毡布 + 真实扑克牌”的视觉方向。整体应避免简单表格化或纯按钮化 UI，优先让用户第一眼看出这是一个可玩的斗地主牌桌。

| 项目 | 设计要求 |
|------|----------|
| 桌面氛围 | 使用深色木质环境、绿色毡布中心区、金色细线和低饱和阴影形成牌桌层次 |
| 扑克牌 | 牌面采用白色纸张、微弱纸纹、红黑花色、角标、中心大花色；王牌采用竖排 JOKER 与专属红/黑标识 |
| 王牌图像 | 大王、小王牌面使用人物形象图，不在牌面额外显示“大王/小王”文字 |
| 牌背 | 使用红色或深色牌背，包含重复纹理、内边框和中心标记，明确区别于正面牌 |
| 交互反馈 | 选中手牌上移、阴影增强、金色描边；当前行动玩家卡片高亮；AI 思考状态有可感知提示 |
| 身份头像 | 玩家身份确定后使用地主/农民头像强化阵营识别，头像与剩余牌数、身份文案保持同屏可见 |
| 信息密度 | 顶部只保留阶段、当前玩家、倍数、倒计时和地主牌；操作按钮集中在右下侧，出牌记录保持可扫读 |
| 响应式 | 桌面端优先横向牌桌；移动竖屏可堆叠玩家与手牌，保证按钮和手牌可点 |
...
| 操作区 | 根据当前阶段切换按钮 |
| 出牌记录 | 展示最近若干手牌局动作 |
| 牌面图像 | 所有牌使用真实扑克牌风格组件渲染，不允许只用纯文字块表达 |
...
| 手牌展示 | 17-20 张手牌应优先通过叠放压缩在手牌区内展示完整，避免用户开局必须横向拖动才能看全 |
| 点击响应 | 选牌必须单次点击/触摸立即生效；手牌重叠区域不能导致用户需要重复点击同一张牌 |
| 出牌失败 | 保留当前选牌，展示明确错误原因 |
| 不可操作 | 非本人回合时按钮置灰，不响应点击 |
| AI 回合 | 显示“电脑思考中”，短暂延迟后出牌 |
| 炸弹/王炸 | 使用更强视觉和音效反馈，但不阻塞流程 |
| 音乐音效 | 背景音乐默认关闭，音效默认开启；用户可在牌桌顶部快速切换 |
